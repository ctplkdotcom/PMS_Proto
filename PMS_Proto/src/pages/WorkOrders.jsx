...
const TABS = ['All', 'Draft', 'Pending SSD Service Manager Endorsement', 'Pending SSD G&C Review', 'Pending SSD AS Endorsement', 'Under PWD Grouping', 'Pending OIC Review', 'Pending PWD Proceed IAS', 'Submitted to IAS for Tendering', 'Approved IAS', 'In Progress', 'Completed'];
const CATEGORIES = ['All', 'MEP', 'Building', 'Facilities', 'Security'];

const STATUS_STYLES = {
  'Draft': { background: '#F1F5F9', color: '#64748B' },
  'Pending SSD Service Manager Endorsement': { background: '#FFF7ED', color: '#B45309' },
  'Pending SSD G&C Review': { background: '#FFF7ED', color: '#C2410C' },
  'Pending SSD AS Endorsement': { background: '#FFF7ED', color: '#92400E' },
  'Under PWD Grouping': { background: 'var(--info-bg)', color: 'var(--info)' },
  'Pending OIC Review': { background: '#FFF7ED', color: '#C2410C' },
  'Pending PWD Proceed IAS': { background: 'var(--warning-bg)', color: '#B45309' },
  'Submitted to IAS for Tendering': { background: '#F5F3FF', color: '#7C3AED' },
  'Approved IAS': { background: '#F0FDF4', color: '#059669' },
      ...
      <div style={{ display: 'flex', gap: 0, borderBottom: '2px solid var(--border)', marginBottom: 16, overflowX: 'auto', whiteSpace: 'nowrap', scrollbarWidth: 'thin', scrollbarColor: '#CBD5E1 transparent', WebkitOverflowScrolling: 'touch', maskImage: 'linear-gradient(to right, black 92%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to right, black 92%, transparent 100%)' }} className="tabs-scroll">
        {tabs.map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: '10px 18px', fontSize: 13, fontWeight: activeTab === tab ? 700 : 500, color: activeTab === tab ? 'var(--primary)' : '#64748B', background: 'none', border: 'none', borderBottom: activeTab === tab ? '2px solid var(--primary)' : '2px solid transparent', marginBottom: -2, cursor: 'pointer', transition: 'color 0.15s', whiteSpace: 'nowrap', flexShrink: 0 }}>
            {tab} <span style={{ fontSize: 11, color: '#94A3B8', marginLeft: 4 }}>({tab === 'All' ? workOrders.length : workOrders.filter((w) => w.status === tab).length})</span>
          </button>
        ))}
      </div>
